...
async function runPatcher(inputPath, outputPath) {
  const tmpPath = path.join(path.dirname(outputPath), "tmp_remux.mp4");
  try {
    ...
    if (ffmpegRes.status !== 0) throw new Error("FFmpeg faststart remux failed");
    ...
    console.log("Successfully patched:", outputPath);
  } finally {
